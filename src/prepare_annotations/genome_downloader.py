...
from prepare_annotations.vcf_downloader import download_path, list_paths
...
def write_fai_for_fasta(
    ...
    *,
    ...
    # We parse in binary to compute byte offsets accurately.
    ...
    with fasta_path.open("rb") as f, fai_path.open("w", encoding="utf-8") as out:
        while True:
            ...
            if line.startswith(b">"):
                ...
                header = line[1:].strip()
                # Use first token as contig name (htslib behavior).
            ...
            if current_name is None:
                # Skip any junk before the first header.
                continue
            ...
            if not saw_sequence_line:
                # First sequence line defines offset/line widths.
...
def ensure_uncompressed_fasta_with_fai(
    ...
    *,
    ...
    if fasta_gz_path.suffix != ".gz":
        # Already uncompressed; index it in-place.
        fasta_path = fasta_gz_path
    else:
        # Remove the ".gz" suffix (e.g. ".fa.gz" -> ".fa")
...
def find_genome_file(
    ...
    with start_action(
        ...
        # Should typically be exactly one match
        if len(files) > 1:
            action.log(
...
def download_ensembl_genome(
    ...
    Examples:
        ...
        >>> path = download_ensembl_genome()
        
        # Download specific release
        >>> path = download_ensembl_genome(release=114)
        
        # Download soft-masked toplevel
        >>> path = download_ensembl_genome(
        ...     genome_type=GenomeType.TOPLEVEL,
        ...     masking=MaskingType.SOFTMASKED
        ... )
        
        # Download a single chromosome
        >>> path = download_ensembl_genome(
        ...     genome_type=GenomeType.CHROMOSOME,
        ...     chromosome="21"
        ... )
    """
    ...
    with start_action(
        ...
        # Download using the robust downloader from vcf_downloader
    ...
    with start_action(
        ...
        if chromosomes is not None:
            ...
            for f in files:
                ...
                # Pattern: Species.Assembly.dna.chromosome.{CHR}.fa.gz
